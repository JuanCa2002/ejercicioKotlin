...
fun getSumAllAccountsAllClients():Double {
    val balancesClientes= customers.map { it.calculateBalance() }
    return balancesClientes.sumOf { it }
}
/**
 * retorna la suma total del dinero del banco en cuentas de cheques
 */
fun getSumCheckAccountsAllClients():Double {
    val balancesClientesCuentasCorrientes= customers.map { it.calculateBalanceCheckAccounts() }
    return balancesClientesCuentasCorrientes.sumOf { it }
}
...
fun getSumSavingAccountsAllClients():Double {
    val balancesClientesCuentasAhorros= customers.map { it.calculateBalanceSavingAccounts() }
    return balancesClientesCuentasAhorros.sumOf { it }
}
...
fun getCities():List<String> {
  val ciudades= customers.map { it.city }
  return ciudades.distinctBy { it }
}

fun getCustomersByCity(city: String): List<Customer>? {
    val personas= customers.filter { it.city== city }
    return personas
}
...
fun getBalanceByCities(): Map<String,Double> {
    val ciud=  customers.map { it.city }
    val city= customers.groupBy { it.city }
    val ciudadesBalances= mapOf<String,Double>("bogota" to customers.filter{it.city=="bogota"}.sumOf { it.accounts.sumOf { it.balance } },
                                               "armenia" to customers.filter{it.city=="armenia"}.sumOf { it.accounts.sumOf { it.balance } },
...
fun getCustomerByName(name: String): Customer? {
    val cliente= customers.find {it.name==name}
    return cliente
...
fun sortCustomerByBalanceDesc(): List<Customer> {
    val clientesBalanceMenorMayor= customers.sortedByDescending{ it.calculateBalance() }
    return clientesBalanceMenorMayor
}